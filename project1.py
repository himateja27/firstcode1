...
con=db.connect(user="root",password="7330870063",host="localhost",database="BankAccount1")

def createaccount(acc_no,name,balance):
    cur=con.cursor()
    quary='insert into bankaccount values(%s,%s,%s)'
...
def Deposite(acc_no,amount):
    cur=con.cursor()
    quary='update bankaccount set balance=balance+%s where acc_no=%s'
...
def withdraw(acc_no,amount):
    cur=con.cursor()
    quary='select balance from bankaccount where acc_no=%s'
    cur.execute(quary,(acc_no,))
    result=cur.fetchone()[0]
    if result>=amount:
        data='update bankaccount set balance=balance-%s where acc_no=%s'
